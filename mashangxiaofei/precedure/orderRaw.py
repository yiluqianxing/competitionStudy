...
def mygroupCount(group):
    ...
    #最近一次购物和最远的一次购物时间
    orderInfo_buy_time_latest = np.max(group.orderInfo_timestamp)
    orderInfo_buy_time_Oldest = np.min(group.orderInfo_timestamp)
    # 平均每次购物多少钱，购物金额最大的一笔
    orderInfo_buy_money_max = np.max(group.amt_order)
    orderInfo_buy_money_min = np.min(group.amt_order)
    orderInfo_buy_money_ave = (group.amt_order.sum()) / count
    orderInfo_buy_money_std = np.std(group.amt_order)
    orderInfo_buy_money_maxCutmin = orderInfo_buy_money_max - orderInfo_buy_money_min
    orderInfo_buy_money_maxCutave = orderInfo_buy_money_max - orderInfo_buy_money_ave

    #统计用户在16,17,15  年总共花了多少钱
    tmp = group.ix[group.orderInfo_year == '2017']
    orderInfo_buy_money_total2017 = np.sum(tmp.amt_order)
    tmp = group.ix[group.orderInfo_year == '2016']
    orderInfo_buy_money_total2016 = np.sum(tmp.amt_order)
    tmp = group.ix[group.orderInfo_year == '2015']
    orderInfo_buy_money_total2015 = np.sum(tmp.amt_order)
    ...
    indexs = {
        ...
        "orderInfo_buy_time_latest":orderInfo_buy_time_latest,
        "orderInfo_buy_time_Oldest":orderInfo_buy_time_Oldest,
        "orderInfo_buy_money_max": orderInfo_buy_money_max,
        "orderInfo_buy_money_min": orderInfo_buy_money_min,
        "orderInfo_buy_money_ave": orderInfo_buy_money_ave,
        "orderInfo_buy_money_std": orderInfo_buy_money_std,
        "orderInfo_buy_money_maxCutmin": orderInfo_buy_money_maxCutmin,
        "orderInfo_buy_money_total2017": orderInfo_buy_money_total2017,
        "orderInfo_buy_money_total2016": orderInfo_buy_money_total2016,
        "orderInfo_buy_money_total2015": orderInfo_buy_money_total2015,
        "orderInfo_buy_money_maxCutave": orderInfo_buy_money_maxCutave,
    }
...
if __name__ == '__main__':
    fulldata = pd.read_csv("../input/fulldata_oreder_undealhebingorder.csv", low_memory=False)
    ...
    num = 167960
    bili = 0.1
    fulldata = fulldata.ix[fulldata.sts_order_number == 1]
    fulldata.orderInfo_year = fulldata.orderInfo_year.astype('str')
    # print(fulldata.orderInfo_year.value_counts())
    # print("kk",fulldata.ix[fulldata.orderInfo_year == '2017'])
    ...
    tmp122.to_csv("../dealinput/orderCount430.csv", index=False)
